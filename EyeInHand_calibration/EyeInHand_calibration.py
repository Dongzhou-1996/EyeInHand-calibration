...
def generate_object_points_in_3d(chesssboard_aspect_ratio=(6, 9), unit_size=0.0025):
...
def tcp_x(step=0.05, interval=1):
    tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_socket.connect(server_addr)
...
def tcp_y(step=0.05, interval=1):
    tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_socket.connect(server_addr)
    data = tcp_socket.recv(1116)
    position = struct.unpack('!6d', data[444:492])
...
def tcp_z(step=0.05, interval=1):
    tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_socket.connect(server_addr)
    data = tcp_socket.recv(1116)
    position = struct.unpack('!6d', data[444:492])
...
def robot_arm_initialization(init_pos=[-0.15747745, 0.44148789, 0.63479836, -1.24728887, -0.66890486, 0.88747505]):
    tcp_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_socket.connect(server_addr)
...
parser.add_argument('--data_num', type=int, default=12, help='the number of acquisition')
